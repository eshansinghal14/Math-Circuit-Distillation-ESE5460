...
``\texttt{$a$+$b$=}'' (22\_add), batch 32, AdamW with a peak rate of $10^{-6}$, ten warmup steps
and cosine decay to a tenth of the peak, 100 steps.
...
$10^{-6}$, batch size 32, $\tau = 1$, the same data order, and the unfrozen arg-token and DLA
...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    optimiser & AdamW, peak learning rate $10^{-6}$, linear warmup over 10 steps, \\
              & cosine decay to $10^{-7}$ at the last step, gradient clipping at $1.0$ \\
