...
\begin{equation}
  ...
  \label{eq:total}
\end{equation}
where $\mathcal{P}$ is the set of positions that predict an answer token or the end-of-sequence
token after it, $z_p$ the logits at position $p$, $\tau = 1$, and $\lambda = 0.1$
...
of top tokens whose probabilities under $\softmax(z/\tau)$, $\tau = 1$, sum to $0.95$, capped at
...
``\texttt{$a$+$b$=}'' (22\_add), batch 32, AdamW with a peak rate of $5 \times 10^{-7}$, ten
warmup steps and cosine decay to $0.3$ of the peak, 150 steps.
...
KD from the teacher's logits at $\tau = 1$; KD plus the graph loss \eqref{eq:total} with
...
supervised fine-tuning on the same prompts. Everything else is held fixed: the schedule of
Table~\ref{tab:config}, batch size 32, $\tau = 1$, the same data order, and the unfrozen arg-token and DLA
supernodes of Section~\ref{sec:supernodes-general} with three neurons each
(Appendix~\ref{app:details} has the full table). The graph runs train for 150 steps; the control
...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    training data & 22\_add prompts ``\texttt{$a$+$b$=}'', batch 32, 150 steps (baselines: 300) \\
    optimiser & AdamW, peak learning rate $5 \times 10^{-7}$, linear warmup over 10 steps, \\
              & cosine decay to $1.5 \times 10^{-7}$ at the last step, gradient clipping at $1.0$ \\
    precision & fp32 master weights and optimiser moments, bf16 autocast forward; teacher bf16 \\
    KD term & $\tau = 1$, forward KL over the positions predicting the answer tokens and EOS \\
