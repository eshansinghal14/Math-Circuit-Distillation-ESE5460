window.utilCg = (function(){
  ...
  async function formatData(data, visState){
    ...
    var idToNode = {}
    var maxLayer = d3.max(nodes.filter(d => d.feature_type != 'logit'), d => +d.layer) || 0
    ...
    nodes.forEach(d => {
      ...
      var inputAbsDenom = d.inputAbsSum || 1
      d.sourceLinks.forEach(e => e.pctInput = e.weight/inputAbsDenom)
      d.inputError = d3.sum(d.sourceLinks.filter(e => e.sourceNode.isError), e => Math.abs(e.weight))
      d.pctInputError = d.inputError/inputAbsDenom
    })
