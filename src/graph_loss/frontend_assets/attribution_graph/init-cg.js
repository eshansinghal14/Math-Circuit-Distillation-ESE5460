window.initCg = async function (sel, slug, {clickedId, clickedIdCb, isModal, isGridsnap, pruningThreshold} = {}){
  ...
  if (!visState.clickedId || visState.clickedId == 'null' || visState.clickedId == 'undefined') {
    visState.clickedId = (data.nodes.find(d => d.isLogit) || data.nodes[0])?.node_id
  }
  ...
  function colorLinks() {
    var absMax = d3.max(data.links, d => d.absWeight) || 1
