...
class HFLlamaGraphAdapter:
    ...
    def _compute_layer_neuron_data_selective(
        ...
        """Memory-efficient variant: avoids [n_pos, d_mlp, d_model] intermediates.

        Computes source-vector norms in O(n_pos * d_mlp) space, selects the
        top-k neurons, then materialises full vectors only for those k neurons.

        Returns:
            keep:    [k] flat indices into n_pos * d_mlp
            acts:    [k] neuron activations (BOS-zeroed)
            te_kept: [k, d_model] target encoders for selected neurons
            sv_kept: [k, d_model] source vectors for selected neurons
        """
        gate_rows, up_rows, out_rows, gate_bias, up_bias = self._layer_weights(
            layer_idx,
            device=mlp_input.device,
        ...
        # BOS masking (zero position 0 so it's never selected)
        ...
        # Source-vector norms without 3-D tensor
        out_norms = out_rows.norm(dim=-1)                  # [d_mlp]
        sv_norms  = neuron_acts.abs() * out_norms.unsqueeze(0)  # [n_pos, d_mlp]
        ...
        # Materialise vectors only for kept neurons
