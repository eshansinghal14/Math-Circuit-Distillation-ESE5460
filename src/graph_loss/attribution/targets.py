...
class AttributionTargets:
        ...
        *,
        ...
        Args:
            attribution_targets: Target specification in one of four formats:
                - None: Auto-select the top-k logits by probability
    ...
    def _compute_logit_vecs(
        ...
        if unembed_proj.ndim != 2 or unembed_proj.shape[1] != d_vocab:
            raise ValueError(
                f"Expected TransformerLens unembed shape (d_model, d_vocab={d_vocab}), "
                f"got {tuple(unembed_proj.shape)}"
            )
