...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Greedy eval during training (default: utils.EVAL_MAX_NEW_TOKENS={EVAL_MAX_NEW_TOKENS})",
    )
    parser.add_argument("--step-log-interval", type=int, default=50,
                        help="Print in-epoch step loss every N batches (default 50)")
    # Save/resume
    parser.add_argument(
        "--save-dir",
        default=os.path.join(os.path.dirname(__file__), "..", "..", "results"),
        metavar="DIR",
        help="Base results directory. New runs create neuron-cluster/<run-name|datetime>/ inside it.",
    )
    parser.add_argument("--run-name", default=None, metavar="NAME",
                        help="Custom folder name for a new run (default: YYYY-MM-DD_HH-MM-SS).")
    parser.add_argument(
        "--resume", action="store_true",
        help="Resume an existing run. Use --checkpoint-run to pick a specific run "
             "(just the datetime); if omitted the most recently modified run is used.",
    )
    parser.add_argument(
        "--checkpoint-run", default=None, metavar="DATETIME",
        help="Datetime folder of the run to resume (e.g. '2026-04-07_22-15-56'). "
             "'neuron-cluster/' is prepended automatically. Only used with --resume.",
    )
    parser.add_argument(
        "--checkpoint-type", default="latest", metavar="latest|best|final|N",
        help="'latest' → highest student_epoch_N, 'best'/'final' → those folders, "
             "or integer N → student_epoch_N. (default: latest)",
    )
